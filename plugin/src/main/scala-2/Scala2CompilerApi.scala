...
object Scala2CompilerApi {
  ...
  def instance(
    ...
    new Scala2CompilerApi[global.type] {
      ...
      def className(
        clazz: Clazz
      ): String = clazz.merge.name.toString
      ...
      def enclosingObjectName(
        enclosingObject: EnclosingObject
      ): String = enclosingObject.name.toString
      ...
      def methodNames(
        clazz: Clazz
      ): List[String] = clazz.merge.impl.body.collect {
      ...
      def isCaseClass(
        clazz: Clazz
      ): Boolean = clazz.merge.mods.isCase
